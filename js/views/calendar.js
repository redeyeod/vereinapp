...
const CalendarView = {
     ...
     */
    render(container) {
        // Sicherheits-Check: Falls Store oder Events noch nicht bereit sind
        if (typeof Store === 'undefined' || !Store.state) {
            container.innerHTML = `<div class="p-10 text-center opacity-50">Lade Daten...</div>`;
            return;
        }
    ...
    renderEventCard(e, canManage) {
        ...
        const containerClass = isPast 
            ? "bg-dark-bg/30 border border-dark-border/50 opacity-60 grayscale-[0.8] hover:grayscale-0 hover:opacity-100 hover:bg-dark-card transition-all" 
            : "bg-dark-card hover:bg-dark-hover border border-dark-border shadow-sm hover:border-brand-500/30 transition-all";
            
        const titleClass = isPast
            ? "text-dark-muted line-through decoration-white/30"
            : "text-white font-bold truncate pr-2 group-hover:text-brand-400";
            
        const badgeColor = isPast ? "text-dark-muted" : "text-brand-500";
        const badgeBorder = isPast ? "border-dark-border/50" : "border-dark-border group-hover:border-brand-500/30";
        ...
        return `
            <div onclick="CalendarView.openDetailModal(${e.id})" class="${containerClass} p-4 rounded-2xl flex items-center gap-4 cursor-pointer group relative overflow-hidden transition-all shadow-sm">
                <!-- Hover Effect Line (Nur bei zukünftigen Events prominent) -->
                ${!isPast ? '<div class="absolute left-0 top-4 bottom-4 w-1 bg-brand-500 rounded-r-full opacity-0 group-hover:opacity-100 transition-opacity"></div>' : ''}

                <!-- Date Badge -->
                <div class="bg-dark-bg border ${badgeBorder} rounded-xl p-2 text-center min-w-[64px] transition-colors flex-shrink-0">
                    <div class="text-[10px] font-bold uppercase ${badgeColor}">${dateDisplayMonth}</div>
                    <div class="text-xl font-bold text-white leading-none mt-0.5 ${isPast ? 'opacity-70' : ''}">${dateDisplayDay}</div>
                    ${rangeBadge}
                </div>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex justify-between items-start">
                        <h4 class="${titleClass} text-base transition-colors">${e.title}</h4>
                    ...
                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-dark-muted mt-1.5 items-center">
                        <span class="flex items-center"><i class="fa-regular fa-clock mr-1.5 ${isPast ? 'text-dark-muted' : 'text-brand-500/70'}"></i> ${e.allDay ? 'Ganztägig' : (e.time || 'Zeit n.a.')}</span>
                        ${e.location ? `<span class="flex items-center truncate max-w-[140px]"><i class="fa-solid fa-location-dot mr-1.5 ${isPast ? 'text-dark-muted' : 'text-brand-500/70'}"></i> ${e.location}</span>` : ''}
                        ${isPast ? '<span class="ml-auto text-[10px] uppercase font-bold tracking-wider border border-dark-border px-1.5 rounded bg-dark-bg/50">Vorbei</span>' : ''}
                    </div>
                </div>
    ...
    openDetailModal(id) {
        ...
        if(modalContainer) {
            modalContainer.classList.remove('max-w-md', 'p-0');
