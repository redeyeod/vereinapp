...
const App = {
    ...
    init() {
        ...
        // 1. Daten-Store initialisieren
        if (typeof Store !== 'undefined') {
            Store.init();

            // Reaktivität für Notifications
    ...
    loadCurrentUser() {
        const id = localStorage.getItem('vm_current_user_id');
        if(id && typeof Store !== 'undefined') {
            this.state.currentUser = Store.state.members.find(m => m.id == id);
        }
    },
    ...
    can(action) {
        ...
        const permissions = {
            // Admin darf alles bearbeiten, löschen und erstellen
            ...
            // Mitglieder haben nur Leserechte (leeres Array für spezielle Aktionen)
            member:  [] 
        };
            ...
            <div id="settings-modal" class="p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 rounded-xl bg-dark-bg border border-dark-border text-center mt-6">
                        ...
                        <button onclick="if(confirm('Alle lokalen Daten löschen?')) { localStorage.clear(); location.reload(); }" class="text-xs text-red-400 hover:underline">
    ...
    updateNotificationDot() {
        ...
        // Prüfe News
        if (Store.state.news.some(n => new Date(n.date).getTime() > lastRead)) {
            hasNew = true;
        }
