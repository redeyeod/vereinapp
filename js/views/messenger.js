...
const MessengerView = {
    ...
    renderInputArea() {
        ...
        return `<div class="min-h-[62px] ${C.headerBg} px-4 py-2 flex flex-col justify-end z-30 relative shrink-0 border-l border-[#2a3942]">${this.renderAttachMenu()}${this.renderEmojiPicker()}${replyMsg ? `<div class="flex items-center justify-between bg-[#1f2c34] p-2 rounded-t-lg border-l-4 border-[#00a884] mb-1 animate-scale-in"><div class="text-sm text-[#8696a0]"><p class="text-[#00a884] font-bold text-xs mb-0.5">${replyMsg.sender}</p><p class="truncate max-w-[200px]">${replyMsg.text}</p></div><button onclick="MessengerView.cancelReply()" class="text-[#8696a0] hover:text-white p-2"><i class="fa-solid fa-times"></i></button></div>` : ''}<div class="flex items-end gap-2 w-full"><button onclick="MessengerView.toggleAttachMenu()" class="mb-3 text-[#8696a0] hover:text-[#d1d7db] transition w-8 text-center text-xl"><i class="fa-solid fa-plus"></i></button><form onsubmit="MessengerView.sendMessage(event)" class="flex-1 flex items-end gap-2 mb-1.5"><div class="flex-1 bg-[#2a3942] rounded-lg flex items-end min-h-[42px] py-2 px-3 relative"><button type="button" onclick="MessengerView.toggleEmojiPicker()" class="text-[#8696a0] hover:text-[#ffde34] transition mr-3 text-lg mb-0.5"><i class="fa-regular fa-face-smile"></i></button><input type="text" name="message" id="chat-input" autocomplete="off" placeholder="Nachricht eingeben" class="bg-transparent border-none text-[#d1d7db] text-sm w-full focus:outline-none placeholder-[#8696a0] max-h-32 overflow-y-auto leading-relaxed"></div><button type="submit" class="w-10 h-10 flex items-center justify-center rounded-full ${C.accentColor} ${C.accentColorHover} text-white shadow-md transition-transform active:scale-95 mb-0.5"><i class="fa-solid fa-paper-plane text-sm pl-0.5"></i></button></form></div></div>`;
    },
    ...
    renderMessageBubble(msg) {
        ...
        if (msg.replyToId) {
            ...
            if (parent) replyHtml = `<div class="bg-black/20 rounded p-1.5 mb-1 text-xs border-l-4 border-[#00a884]/50 cursor-pointer hover:bg-black/30 transition" onclick="document.getElementById('msg-${parent.id}')?.scrollIntoView({behavior:'smooth'})"><span class="text-[#00a884] font-bold block mb-0.5">${parent.sender}</span><span class="text-white/70 truncate block">${parent.text}</span></div>`;
        }

        let reactionsHtml = '';
        if (msg.reactions && msg.reactions.length > 0) {
            reactionsHtml = `<div class="absolute -bottom-2 ${isMe ? 'right-2' : 'left-2'} bg-[#1f2c34] rounded-full px-1.5 py-0.5 border border-[#0b141a] flex gap-0.5 shadow-sm text-[10px] z-10 cursor-pointer" onclick="alert('${msg.reactions.map(r => r.u).join(', ')}')">${msg.reactions.map(r => `<span>${r.e}</span>`).join('')}${msg.reactions.length > 1 ? `<span class="text-[#8696a0] ml-0.5">${msg.reactions.length}</span>` : ''}</div>`;
        }
        ...
        return `<div id="msg-${msg.id}" class="flex flex-col ${align} mb-2 group max-w-full relative select-none"><div class="relative max-w-[85%] md:max-w-[65%] shadow-sm ${bubbleColor} ${C.textMain} px-2 pt-2 pb-1 ${tailClass} rounded-lg ${msg.isPinned ? 'border border-yellow-500/30' : ''}">${msg.isPinned ? '<i class="fa-solid fa-thumbtack text-[10px] text-yellow-500 absolute -top-1.5 -right-1 rotate-45 drop-shadow-md"></i>' : ''}<button onclick="MessengerView.toggleMsgMenu('${contextMenuId}')" class="absolute top-0 right-0 w-8 h-6 bg-gradient-to-b from-black/20 to-transparent rounded-tr-lg opacity-0 group-hover:opacity-100 transition-opacity text-right pr-2 pt-1 text-white/80 hover:text-white z-10"><i class="fa-solid fa-angle-down text-xs drop-shadow-md"></i></button><div class="px-1 pb-1 relative z-0 break-words min-w-[80px]">${replyHtml}${!isMe && this.state.activeType === 'group' ? `<p class="text-xs font-bold text-[#eeb346] mb-1">${msg.sender}</p>` : ''}${contentHtml}</div><div class="float-right flex items-end gap-1 ml-2 -mb-0.5 opacity-60"><span class="text-[10px] text-[#e9edef]">${new Date(msg.time).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span>${isMe ? '<span class="text-[#53bdeb] text-[10px]"><i class="fa-solid fa-check-double"></i></span>' : ''}</div>${reactionsHtml}</div><div id="${contextMenuId}" class="hidden absolute top-8 ${isMe ? 'right-4' : 'left-4'} bg-[#233138] border border-[#2a3942] rounded-lg shadow-2xl z-50 w-48 animate-scale-in py-1"><div class="flex justify-around p-2 border-b border-[#2a3942] bg-[#1f2c34]">${['👍','❤️','😂','😮','🙏'].map(e => `<button onclick="MessengerView.reactToMessage('${msg.id}', '${e}')" class="hover:scale-125 transition text-lg">${e}</button>`).join('')}</div><button onclick="MessengerView.replyTo('${msg.id}')" class="w-full text-left px-4 py-2.5 text-sm text-[#e9edef] hover:bg-[#111b21] flex gap-3 items-center"><i class="fa-solid fa-reply w-4"></i> Antworten</button><button onclick="MessengerView.copyMessageText('${msg.text}')" class="w-full text-left px-4 py-2.5 text-sm text-[#e9edef] hover:bg-[#111b21] flex gap-3 items-center"><i class="fa-regular fa-copy w-4"></i> Kopieren</button><button onclick="MessengerView.pinMessage('${msg.id}')" class="w-full text-left px-4 py-2.5 text-sm text-[#e9edef] hover:bg-[#111b21] flex gap-3 items-center"><i class="fa-solid fa-thumbtack w-4"></i> ${msg.isPinned ? 'Lösen' : 'Anpinnen'}</button><button onclick="MessengerView.forwardMessage('${msg.id}')" class="w-full text-left px-4 py-2.5 text-sm text-[#e9edef] hover:bg-[#111b21] flex gap-3 items-center"><i class="fa-solid fa-share w-4"></i> Weiterleiten</button>${isMe ? `<button onclick="MessengerView.editMessage('${msg.id}')" class="w-full text-left px-4 py-2.5 text-sm text-[#e9edef] hover:bg-[#111b21] flex gap-3 items-center"><i class="fa-solid fa-pen w-4"></i> Bearbeiten</button>` : ''}${isMe || App.can('admin') ? `<button onclick="MessengerView.deleteMessage('${msg.id}')" class="w-full text-left px-4 py-2.5 text-sm text-red-400 hover:bg-[#111b21] flex gap-3 items-center"><i class="fa-solid fa-trash w-4"></i> Löschen</button>` : ''}</div></div>`;
    },
    ...
    replyTo(msgId) { this.state.replyingTo = msgId; this.render(document.getElementById('content')); setTimeout(() => document.getElementById('chat-input')?.focus(), 50); },
    cancelReply() { this.state.replyingTo = null; this.render(document.getElementById('content')); },
    editMessage(msgId) { const msg = this.findMessage(msgId); if(msg) { const input = document.getElementById('chat-input'); if(input) { input.value = msg.text; input.focus(); this.state.editingId = msgId; } } },
    copyMessageText(text) { navigator.clipboard.writeText(text); if(window.App) window.App.showToast("Kopiert!"); },
    reactToMessage(msgId, emoji) { this.updateMsgProperty(msgId, (msg) => { if(!msg.reactions) msg.reactions = []; const myName = Store.state.currentUser.firstName; const existing = msg.reactions.find(r => r.u === myName && r.e === emoji); if(existing) msg.reactions = msg.reactions.filter(r => r !== existing); else msg.reactions.push({ u: myName, e: emoji }); }); },
